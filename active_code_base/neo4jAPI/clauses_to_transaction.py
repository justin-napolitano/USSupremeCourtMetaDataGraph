...
def get_transaction_df(df = None):  
    ...
    try:
        ...
        df['transaction'] = df.apply(lambda x: neo.neoAPI.create_clause_node(name= x['clause'],  
        topic = x['topic'], 
        citation = x['citation']),
        axis = 1
        )
...
if __name__ == "__main__":
    ...
    cwd = get_cwd()
    file_list = get_files(cwd = cwd, input_directory = 'clauses_data')
    #pprint(file_list)
    ...
    outfile = write_transaction_to_file(df = df , cwd = cwd, file_name = 'clauses_transaction_df.csv')
